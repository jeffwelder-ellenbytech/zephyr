...
/*
 * A scripted stand-in for the Bluetooth host's GATT client API.
 *
 * CONFIG_BT_HCI is not set, so subsys/bluetooth/CMakeLists.txt never adds the host
 * directory and these are the only definitions of the GATT client entry points in the
 * image. The same technique, and for the same reason, as tests/bluetooth/audio.
 *
 * Every entry point below reproduces the control flow of its counterpart in
 * subsys/bluetooth/host/gatt.c, because the transport under test is built almost entirely
 * out of claims about exactly when the host calls back:
 *
 *   bt_gatt_subscribe()   issues the client characteristic configuration write and only
 *                         then links the node, so a failed write leaves nothing linked.
 *   bt_gatt_unsubscribe() issues its own write, unlinks the node and returns success
 *                         WITHOUT a synchronous notify(NULL) - that arrives later, from
 *                         the write response.
 *   gatt_write_ccc_rsp()  runs the subscribe callback, then removes the node and reports
 *                         it through notify(NULL); on an error it first looks the
 *                         connection's subscription list up and returns silently if the
 *                         node has already left it.
 *   disconnect            fails every pending ATT request with -ECONNRESET first (which
 *                         is what ends a discovery and a descriptor write), then sweeps
 *                         the volatile subscriptions with notify(NULL), and only then
 *                         runs the application's bt_conn_cb.disconnected callbacks.
 *
 * Discovery results and descriptor write responses are delivered from a work queue by
 * default, never from inside the call that asked for them, because the whole generation
 * and ownership machinery in the transport exists to survive callbacks that arrive after
 * the caller has stopped waiting for them.
 */
...
struct gatt_stub_peer gatt_stub_peer;
struct gatt_stub_log gatt_stub_log;
struct gatt_stub_faults gatt_stub_faults;
...
/* The host's per-connection subscription list, which never holds more than the one node
 * this transport owns.
 */
static struct bt_gatt_subscribe_params *sub_node;
static struct bt_conn *sub_conn;

/* The one outstanding ATT request of each kind, as the host's request queue would hold
 * it. Both are answered from a work queue so that no callback ever runs on the thread
 * that asked for it.
 */
static struct {
	struct bt_conn *conn;
...
void gatt_stub_reset(void)
{
	...
	gatt_stub_peer = (struct gatt_stub_peer){
		.svc_handle = 0x0010,
	...
	/* Every leg of every procedure is answered from the work queue, one tick of
	 * latency away, unless a test asks for something else.
	 */
...
/*
 * One leg of a discovery. The service and characteristic legs hand back the declaration
 * attribute rather than the value attribute, exactly as gatt_find_type_rsp() and
 * parse_characteristic() do, which is what makes the transport's "read the value handle
 * out of the declaration" requirement testable.
 */
static void discover_deliver(void)
{
	...
	/* Nothing matched, which every discovery response handler in the host reports
	 * as a terminal callback with a NULL attribute.
	 */
...
static void ccc_deliver(void)
{
	...
	if (att_err != 0U) {
		/* gatt_sub_find(): a connection whose subscription list is empty has no
		 * gatt_sub at all, and the response is dropped without any callback.
		 * That is the case a detach's unsubscribe leaves behind, and the only
		 * thing left that can release the transport's claim on the parameters
		 * is then the disconnect itself.
		 */
	...
	if (params->value == 0U) {
		/* The write that turned notifications off has landed, so the node has
		 * finally left the host for good.
		 */
		params->notify(conn, params, NULL, 0);
	}
}
...
int bt_gatt_subscribe(struct bt_conn *conn, struct bt_gatt_subscribe_params *params)
{
	...
	if (sub_node == params) {
		/* The host reports a node that is already linked and drops its own
		 * bookkeeping, leaving the node in place.
		 */
		return -EALREADY;
	}
	...
	if (gatt_stub_faults.subscribe_already_enabled) {
		/* gatt.c skips the write when a subscription already covers the handle,
		 * and then owes no callback at all.
		 */
		return 0;
	}
...
int bt_gatt_unsubscribe(struct bt_conn *conn, struct bt_gatt_subscribe_params *params)
{
	...
	if (gatt_stub_faults.unsubscribe_err != 0) {
		/* gatt_write_ccc() failed, so the node stays in the list and no
		 * callback of any kind is owed.
		 */
		return gatt_stub_faults.unsubscribe_err;
	}

	if (ccc_req.active && ccc_req.params == params) {
		/* bt_gatt_cancel(): a descriptor write still in flight for these
		 * parameters is answered synchronously, from inside this call, with
		 * BT_ATT_ERR_UNLIKELY, before the unsubscribe goes any further.
		 */
		(void)k_work_cancel_delayable(&ccc_work);
		ccc_req.att_err = BT_ATT_ERR_UNLIKELY;
		ccc_deliver();
	}
	...
	if (gatt_stub_faults.subscribe_already_enabled) {
		/* No write when another subscription still covers the handle; the host
		 * reports the removal synchronously instead.
		 */
		params->notify(conn, params, NULL, 0);
		return 0;
	}

	/* The node leaves the list here, but the write it triggered is still in flight
	 * and its response is what eventually reports the removal.
	 */
	atomic_set_bit(params->flags, BT_GATT_SUBSCRIBE_FLAG_WRITE_PENDING);
...
void gatt_stub_disconnect(struct bt_conn *conn)
{
	...
	/* Buffers still queued for transmission are dropped, and their completion
	 * callbacks never run.
	 */
	tx_done_count = 0;

	/*
	 * att_reset(): every pending ATT request is failed with -ECONNRESET. For a
	 * discovery that means a terminal callback with a NULL attribute, and for a
	 * descriptor write it means gatt_write_ccc_rsp() with an ATT error.
	 */
	(void)k_work_cancel_delayable_sync(&discover_work, &sync);
	(void)k_work_cancel_delayable_sync(&ccc_work, &sync);
	...
	/*
	 * bt_gatt_disconnected() -> remove_subscriptions(): this transport's node is
	 * marked BT_GATT_SUBSCRIBE_FLAG_VOLATILE, so whatever is still linked is
	 * removed and reported through notify() with NULL data.
	 */
	if (sub_node != NULL && sub_conn == conn) {
	...
	/* notify_disconnected(): the application's callbacks, last of all. */

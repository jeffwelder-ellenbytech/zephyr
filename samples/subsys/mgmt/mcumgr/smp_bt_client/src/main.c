...
/*
 * Uploads a firmware image to a Bluetooth peer that runs the SMP service, with this board
 * in the central role. The image is read from a littlefs file system on the board's
 * external QSPI flash, so nothing about it is baked into this binary.
 */
...
/*
 * The one reference this sample holds on the peer. Both the connected callback, on a
 * failed connection, and main, when it gives up waiting, want to release it, so whoever
 * swaps the pointer out first owns the release and the other side finds nothing to do.
 */
static atomic_ptr_t peer;
static K_SEM_DEFINE(peer_connected, 0, 1);

/* Matches the SMP service UUID that every Zephyr SMP Bluetooth server advertises. */
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	if (err != 0) {
		...
		/* Also reached when main cancels a connection it gave up waiting for; by
		 * then main has already taken the reference, and this finds nothing.
		 */
...
/*
 * Writes a zero filled image carrying nothing but a valid MCUboot image header, so that
 * the sample has something to upload out of the box: a peer running img_mgmt rejects the
 * first upload frame outright unless it starts with the MCUboot magic.
 */
static int placeholder_write(void)
{
	...
	for (off = 0; off < PLACEHOLDER_SIZE; off += CHUNK_SIZE) {
		...
		/* A short write means the file system is full, which leaves a truncated
		 * image behind; report it rather than upload something incomplete.
		 */
...
static int image_upload(size_t image_size)
{
	...
	/* Image 0, and no image hash, which also means that no upload can be resumed. */
	...
	while (sent < image_size) {
		...
		/*
		 * The peer, not the file position, decides where the next frame starts:
		 * it answers every frame with the offset it expects next, and the image
		 * client puts that offset in the frame it sends. Seeking to it keeps the
		 * bytes and the offset they are labelled with in agreement. Reading on
		 * from wherever the file happened to be left would silently send the
		 * wrong bytes for the offset whenever the peer asks to resume anywhere
		 * other than exactly where this side stopped.
		 */
		...
		/* A peer that never advances would otherwise spin here forever. */
		if (sent <= offset) {
...
int main(void)
{
	...
	/* fs_stat() reports -ENOENT both for a file that is not there and for a mount
	 * point that is not there, and the two want completely different fixes, so rule
	 * the second one out before reading anything into the first.
	 */
	...
	/* An upload of nothing at all would otherwise run to completion without a single
	 * frame reaching the peer, and report success.
	 */
	...
	if (conn == NULL) {
		/* The scan callback is still armed unless it stopped the scan itself,
		 * and nothing is waiting on it any more. Left running it would connect
		 * to a peer that turns up later with nobody to drive the link, holding
		 * it open and stopping the peer from advertising to anyone else.
		 */
		(void)bt_le_scan_stop();
		LOG_ERR("No SMP server to connect to");

		conn = atomic_ptr_set(&peer, NULL);
		if (conn != NULL) {
			/* A connection that was still being established when the wait
			 * gave up. Cancel it rather than leave the reference behind.
			 */
	...
	/* Attaching blocks until the peer's SMP service has been discovered and subscribed
	 * to, so it has to run here rather than on a work queue or a Bluetooth callback.
	 */
	...
	if (rc != 0) {
		LOG_ERR("Failed to initialise the SMP client (err %d)", rc);
		goto detach;
	}
	...
	/* No image list buffer: the sample uploads, and never reads the peer's image state. */
